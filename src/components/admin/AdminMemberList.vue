<template>
    <div class="mainContainer">
        <div class="mainHeader">
            <div class="mainTitle">
                <h3>會員管理 - <span class="titleMid">會員列表</span></h3>
            </div>
            <div class="mainOptions">
                ...
                <div v-if="false">
                    <div class="btn btn-sm btn-success" @click="doAction(0)">新增管理員</div>
                </div>
            </div>
        </div>
        ...
        <div class="zeroRow" v-if="memberCount == 0">
            <div>
                <h4>未尋獲任何資料...</h4>
            </div>
        ...
        <div class="listContainer" v-if="memberCount > 0">
            <div class="resultRows">
                <div class="showRemoved">
                    <div class="reviveBtn btn btn-sm" :class="{ 'enableRevive': !showAll, 'btn-primary': showAll }"
                        @click="showRemovedData">顯示已刪除資料</div>
                </div>
                <div class="resultCount">資料總筆數：<span class="resultNumber">{{ memberCount }}</span> 筆</div>
            </div>
            <div class="listTitle">
                <div class="listTitleField"></div>
                <div class="listTitleField">員編</div>
                <div class="listTitleField">姓名</div>
                <!--div class="listTitleField">公司別</!--div//-->
                <div class="listTitleField">部門</div>
                <div class="listTitleField">權限</div>
                <div class="listTitleField">Email</div>
                <div class="listTitleField">操作</div>
            </div>
            <div class="listBody">
                <div class="listLineContainer" v-for="(manager, index) in memberList" :key="index"
                    :class="{ 'removedRecord': manager.enabled == 0 }">
                    <div class="listLine">
                        <div class="listField lineIndex" v-html="index + 1"></div>
                        <div class="listField managerNo" v-html="manager.No"></div>
                        <div class="listField managerName" v-html="manager.name"></div>
                        <!--div class="listField managerCo" v-html="manager.Co"></!--div//-->
                        <div class="listField managerDep" v-html="manager.Dep"></div>
                        <div class="listField managerAuth" v-html="manager.AuthGroupName"></div>
                        <div class="listField managerEmail"><a :href="'mailTo:' + manager.Email" v-html="manager.Email"></a>
                        </div>
                        <div class="listField managerAction" v-if="manager.enabled > 0">
                            <div class="listAction">
                                <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"
                                    class="actionEdit" @click="doAction(manager.ID)">
                                    <title>編輯</title>
                                    <path
                            ...
                            <div class="listAction">
                                <svg xmlns="http://www.w3.org/2000/svg" height="16" width="14" viewBox="0 0 448 512"
                                    class="actionDel" @click="removeManager(manager.ID, manager.name)">
                                    <title>刪除</title>
                                    <path
                        ...
                        <div class="listField managerAction2" v-if="manager.enabled == 0">
                            <div class="listAction">
                                <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26"
                                    class="bi bi-reply-fill dataRestore" viewBox="0 0 16 16"
                                    @click="reviveManager(manager.ID, manager.name)">
                                    <title>復原</title>
                                    <path
...
.mainContainer {
    ...
    .listContainer {
        ...
        .listTitle {
            ...
            grid-template-columns: 30px 180px 180px 180px 180px 480px 1fr;
            column-gap: 1px;
        ...
        .listBody {
            ...
            .removedRecord {
                background-color: rgba(255, 70, 70, .2) !important;
            }
...
const managerTotalPath = config.hostPath + config.adminMemberTotalPath
const memberListPath = config.hostPath + config.adminMemberListPath
...
const queryData = async (setPage: number, setRows: number) => {
    loadingSwitch(true)
    const totalResult: any = await api.callAPI(managerTotalPath, {})
    memberCount.value = totalResult.message
    const queryResult: any = await api.callAPI(memberListPath, { page: setPage, rows: setRows, showAll: showAll.value })
    memberList.value = queryResult.message
    //console.log("Memger Count Receieve => " + memberCount.value)
    //console.log(memberList.value)
    //console.log("mCount => " + memberCount.value + " / showRows => " + showRows.value)
    totalPage.value = Math.ceil(memberCount.value / showRows.value)
...
const doAction = (mid: number) => {
    emit('redirectPage', { managerID: mid, lastPage: currentPage.value }, "modify")
}

const removeManager = (aid: number, aName: string) => {
    ...
    swal.fire(
        ...
        if (result.isConfirmed) {
            const managerRemovePath = config.hostPath + config.adminManagerRemovePath
            const removeManager = async () => {
                ...
                alarm.miniSuccess("管理員 " + aName + " 已刪除")
            }
...
const reviveManager = (aid: number, aName: string) => {
    ...
    swal.fire(
        ...
        if (result.isConfirmed) {
            const managerRevivePath = config.hostPath + config.adminManagerRevivePath
            const reviveManager = async () => {
                ...
                queryData(currentPage.value, showRows.value)
                alarm.miniSuccess("管理員 " + aName + " 已復原")
            }
...
const loadingSwitch = (status: boolean) => {
    //console.log("=== AdminmemberList loading switch ===")
