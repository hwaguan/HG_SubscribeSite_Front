    ...
    <div class="loginContainer">
        ...
        <div class="inputZone">
            <div class="thirdPartyLogin">
                <div class="loginLine">
                    <div class="loginBtn googlelogin">使用 Google 帳號<span v-if="isLogin">登入</span><span
                            v-if="!isLogin">註冊</span></div>
                </div>
                <div class="loginLine">
                    <div class="loginBtn facebooklogin">使用 Facebook 帳號<span v-if="isLogin">登入</span><span
                            v-if="!isLogin">註冊</span></div>
                </div>
                <div class="loginLine">
                    <div class="loginBtn linelogin">使用 Line 帳號<span v-if="isLogin">登入</span><span v-if="!isLogin">註冊</span>
                    </div>
                </div>
            </div>
            <div class="gapLine">
            ...
            <div class="accountInfo">
                <div class="infoLineCase">
                    <div class="infoLine account" :class="{ 'accountError': loginError.account.error }">
                        ...
                        <div class="infoInput">
                            <input type="text" class="memberInfoInput" v-model="loginInfo.account"
                                placeholder="請輸入您的 E-mail" @change="confirmAccount">
                        </div>
                ...
                <div class="infoLineCase">
                    <div class="infoLine password" :class="{ 'passwordError': loginError.pass.error }">
                        ...
                        <div class="infoInput">
                            <input type="text" class="memberInfoInput" v-model="loginInfo.password" maxlength="12"
                                v-if="!hidePass" placeholder="請輸入 6 ~ 12 位密碼" @change="confirmPass">
                            <input type="password" class="memberInfoInput" v-model="loginInfo.password" maxlength="12"
                                v-if="hidePass" placeholder="請輸入 6 ~ 12 位密碼" @change="confirmPass">
                        </div>
                        <div class="infoHide">
                ...
                <div class="infoLineCase">
                    <div class="infoLine confirmPass" :class="{ 'confirmPassError': loginError.cpass.error }">
                        ...
                        <div class="infoInput">
                            <input type="text" class="memberInfoInput" v-model="loginInfo.confirmPass"
                                v-if="!hideConfirmPass" maxlength="12" placeholder="請再次輸入密碼" @change="confirmVerify">
                            <input type="password" class="memberInfoInput" v-model="loginInfo.confirmPass"
                                v-if="hideConfirmPass" maxlength="12" placeholder="請再次輸入密碼" @change="confirmVerify">
                        </div>
                        <div class="infoHide">
            ...
            <div class="extraLine" :class="{ 'atRegister': !isLogin }">
                <div>
                    <div class="register" @click="isLogin = !isLogin"><span v-if="isLogin">註冊帳號</span><span
                            v-if="!isLogin">登入帳號</span></div>
                </div>
                <div v-if="isLogin">
            ...
            <div class="submitBtnCase">
                <div class="submitButton" :class="{ 'btnDisabled': !allPass }" @click="proceedLogin"><span
                        v-if="isLogin">登入</span><span v-if="!isLogin">註冊</span></div>
            </div>
        </div>
    </div>
</template>
...
.loginContainer {
    ...
    .gapLine {
        ...
        margin-left: 10%;
        height: 30px;
        background: linear-gradient(transparent 0px 13px, rgb(210, 210, 210) 13px 13px, transparent 14px 30px);
        display: flex;
    ...
    .accountInfo {
        ...
        .infoLineCase {
            ...
            .infoLine input:focus {
                outline: 2px rgba(0, 183, 255, .3) solid;
            }

            .account {
                background: url('data:image/svg+xml;charset=UTF-8, <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="rgba(0, 0, 0, .1)" class="bi bi-person-fill" viewBox="0 0 16 16"><path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3Zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/></svg>'), linear-gradient(to right, rgba(230, 230, 230, 1) 0px 100px, transparent 150px 100%);
            }

            .accountError {
                background: url('data:image/svg+xml;charset=UTF-8, <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="rgba(0, 0, 0, .1)" class="bi bi-person-fill" viewBox="0 0 16 16"><path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3Zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/></svg>'), linear-gradient(to right, rgba(255, 0, 0, .2) 0px 100px, transparent 150px 100%);
            }

            .password {
                background: url('data:image/svg+xml;charset=UTF-8, <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="rgba(0, 0, 0, .1)" style="margin-top : 5px" class="bi bi-bag" viewBox="0 0 16 16"><path d="M8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1zm3.5 3v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5zM2 5h12v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V5z"/></svg>'), linear-gradient(to right, rgba(230, 230, 230, 1) 0px 100px, transparent 150px 100%);
                /*background-position: 0px 5px !important;*/
            }

            .passwordError {
                background: url('data:image/svg+xml;charset=UTF-8, <svg xmlns="http://www.w3.org/2000/svg" width="50" height="50" fill="rgba(0, 0, 0, .1)" style="margin-top : 5px" class="bi bi-bag" viewBox="0 0 16 16"><path d="M8 1a2.5 2.5 0 0 1 2.5 2.5V4h-5v-.5A2.5 2.5 0 0 1 8 1zm3.5 3v-.5a3.5 3.5 0 1 0-7 0V4H1v10a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V4h-3.5zM2 5h12v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V5z"/></svg>'), linear-gradient(to right, rgba(255, 0, 0, .2) 0px 100px, transparent 150px 100%);
            }
    ...
    .extraLine {
        ...
        background: linear-gradient(transparent 0px 23px, rgb(210, 210, 210) 23px 23px, transparent 24px 30px);
        display: grid;
...
const confirmVerify = () => {
    ...
    if (loginInfo.value.password != loginInfo.value.confirmPass) loginError.value.cpass.text = "確認密碼密碼輸入不一致"
...
const verifyInfo = () => {
    allPass.value = loginInfo.value.account != '' && loginInfo.value.password != '' && loginInfo.value.confirmPass != '' && !loginError.value.account.error && !loginError.value.pass.error && !loginError.value.cpass.error
...
const proceedLogin = () => {
    if (loginInfo.value.account != '' || loginInfo.value.password != '' || loginInfo.value.confirmPass != '') {
        if (!allPass.value) {
            swal.fire(
                {
                    icon: 'warning',
            ...
            alarm.miniMessage(1, greeting + "! " + memberInfo.value.name + " " + GenderText + " 您好!", 2000);
        }
    }
}
</script>